...
<img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp11qy2b9j30vs0qa4qq.jpg" alt="WeChate35b1cf67d7835faeb7aaad797bef898.png" style="zoom:33%;" />
...
<img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp15f5sraj30zu0rqb2a.jpg" alt="WeChat3d6e07d41be67459dd7d9b9497c4881d.png" style="zoom:33%;" />
...
<img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp1bau7ahj31em0punpf.jpg" alt="WeChat6699c193c3b87d971260e41b388f8850.png" style="zoom:33%;" />
...
1. **寻找时间(寻道时间)** ： 
   ...
   - **启动磁头臂**是需要时间的。设耗时s
   - **移动磁头**也需要时间。假设匀速移动，跨越一次磁道耗时m，总共要跨过n条磁道，则：

   **寻道时间 TS = s + m*n**

2.  **延迟时间**

   通过旋转磁盘，磁头定位到目标扇区所需要的时间。设磁盘转速为r(转/秒)，则平均需要的延迟时间**TR=(1/2)*(1/r)=1/2r**

3. **传输时间**

   从磁盘读出或写入所经历的时间。假设转速r，此次读/写的字节数b,每个磁道上的字节数N
...
缺点：如果访问的磁道很分散，性能差，寻道时间长



#### 最短寻找时间优先(SSTF)

**优先处理与当前磁头最近的磁道**，可以保证每次的寻道时间最短，但是并不能保证总的寻道时间最短。(**贪心算法**)
...
最短寻找时间优先算法产生饥饿的原因：磁头在一个小区域内来回地移动，因此有人提出了SCAN算法。



SCAN算法核心只有**磁头移动到最外侧磁道的时候才能往内移动**，**移动到最内侧磁道的时候才能往外移动**。很类似公交车算法。



优点：性能好，不会产生饥饿现象。

缺点：只有**到最边的磁道才能掉头**，事实上，很多时候单边访问请求都执行完了后就能掉头了。 同时，本算法**对各个位置磁道响应频率不平均**。



#### LOOK调度算法

LOOK调度算法(电梯算法)优化了SCAN算法只有到最边才能掉头的缺点。
...
为了解决SCAN算法各个位置磁道响应频率不平均问题，C- SCAN算法规定只有磁头找某个特定方向移动时才处理磁道访问请求，而**返回时直接快速移动至起始端而不处理任意请求**。



优点：比起SCAN算法，对各个位置磁道的响应频率很平均

缺点：只有到达最边上磁道，才改变磁头移动方向
...
磁头读入一个扇区数据后需要一小段时间处理(可能会错过下一个临接的扇区)，如果逻辑上相邻的扇区在物理上也相邻，则读入几个连续的逻辑扇区，可能需要很长的“延迟时间”。
...
<img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp3rcpkqrj30j00j61kx.jpg" alt="WeChatd430aa6a96701a032d673fe07b1c3eb0.png" style="zoom:33%;" />
...
<img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp3xhhe0pj31m00kiu0y.jpg" alt="WeChat639f92693d41c9604dc8ce5eea0818ab.png" style="zoom:33%;" />
...
![WeChat7acfdba41d796d4d87efc1926f39925e.png](http://ww1.sinaimg.cn/large/008aPpVGgy1gnp430af8sj31k60sab2b.jpg)
...
1. **低级初始化(物理初始化)**
   ...
   将磁盘的各个磁道划分扇区。一个扇区分为头、数据区域、尾三个部分。管理扇区所需要的各种数据结构一般存放在头、尾两个部分，包括扇区检验码(如奇偶校验、CRC循环冗余检验码等)

2. 将磁盘分区，每个分区由若干个柱面组成(C、D、E盘等)

   <img src="http://ww1.sinaimg.cn/large/008aPpVGgy1gnp4sx5xbgj30ia0j01kx.jpg" alt="WeChat8ec95d84d4cc063daf5084a5cd53e0cf.png" style="zoom:33%;" />

3. 进行**逻辑格式化**，创建文件系统。包括文件系统的根目录、初始化存储空间管理所需要的数据结构(如 位示图、空闲分区表)



#### 引导块

计算机开机需要进行一系列初始化工作，这些工作通过执行**初始化程序(自举程序)**完成。

初始化程序可以存在ROM(只读存储器)中。ROM中数据在出厂时写入，并且**以后不能再修改**。

ROM一般是出厂就集成在主板上。



自举程序放在ROM，如果需要更新自举程序，会很不方便，因为ROM中数据无法修改。
...
- 完整的自举程序存放在磁盘的启动块(即引导块/启动分区)上，启动块位于磁盘的固定位置
  ...
  拥有启动分区的磁盘称为**启动磁盘或系统磁盘**(C:盘)

开机时，计算机先运行自举装入程序，通过执行该程序可以找到引导块，并将完整的自举程序读入内存，完成初始化。



#### 坏块的管理

坏块属于硬件故障，操作系统无法修复，应该将坏块标记出来，以免错误地使用到它。



对于简单的磁盘，可以在逻辑格式化时(建立文件系统时)对整个磁盘进行坏块检查，表明哪些扇区时坏扇区，比如：在FAT表上标明。(这种方式下，**坏块对操作系统不透明**)
